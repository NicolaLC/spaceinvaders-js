...
Enemies.prototype = {
  ...
  move: function () {
    ...
    this.TARGET_ENEMIES_POSITION.left = Math.min(MAX, Math.max(MIN, offsetLeft));
    if (this.TARGET_ENEMIES_POSITION.left >= MAX - 25 || this.TARGET_ENEMIES_POSITION.left <= MIN + 25) {
  ...
  render: function () {
    ...
    let targetPosLeft = lerp(enemiesWrapper.offsetLeft, TARGET_ENEMIES_POSITION.left, .25);
    let targetPosTop = lerp(enemiesWrapper.offsetTop, TARGET_ENEMIES_POSITION.top, .25);
  ...
  shoot() {
    ...
    const randomEnemy = enemies[Math.floor(Math.random() * enemies.length - 1) + 1];
